...
## 5. "九星连珠得分"——版本差异说明（需要 Codex 注意）

**这是一个命名与实际实现不一致的遗留问题，不是本次修订引入的，是用户最初提供的原始文件里就有的。**

- 框架文件（README.md）里这一节标题叫"进攻信号检查表（九星连珠）"，但实际列出的检查项**只有 5 条**（不是 9 条）：
...
- "九星连珠"字面意思暗示应该有 9 个对齐条件，但目前系统里从未存在过 9 项版本。这个名字大概率是用户沿用了某个更早期/更宏大版本的叫法，实际落地时只做了 5 项。**没有跟用户确认过这个命名差异该怎么处理**（改名对齐实际的 5 项，还是真的把它扩展成 9 项）。

**Codex 处理建议**：使用这个字段前，先跟用户确认是保留"九星连珠"这个名字只是历史遗留叫法（继续用 /5 计分即可），还是真的需要设计出完整的 9 项版本。不要自行假设是哪一种。
...
6. "九星连珠"命名与实际 5 项检查表不符，未跟用户确认如何处理（见第 5 节）。
