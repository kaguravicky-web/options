...
Put Wall/Gamma Flip 和 Max Pain/"杀期权" 回答的是两个不同的问题，尤其在0DTE当天容易被混为一谈：

| | Put Wall / Gamma Flip | Max Pain / "杀期权" |
|---|---|---|
| 回答什么问题 | 波动率状态：这个价位附近做市商对冲行为会压制还是放大波动 | 到期引力：价格有多大动力漂向让最多期权归零的行权价 |
| 强弱跟DTE的关系 | 相对稳定，不随当天时间推进明显变化 | **离到期越近引力越强**，0DTE当天后半段尤其明显 |
| 典型证据 | 价格穿越Wall/Flip Point后波动率变化 | 期权权利金衰减幅度远超标的价格本身涨跌（Theta+Delta双杀） |

**不要用Put Wall单独解释0DTE当天的全部价格行为**——如果观察到"标的只跌1%多，但对应行权价的期权权利金却跌了80%+"，这是Max Pain/时间价值衰减在起作用，跟是否踩在Wall上关系不大，两者需要分开描述，不能笼统归因到一个叙事里。
